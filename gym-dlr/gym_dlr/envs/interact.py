...
class ClusterEnvironment:
    def __init__(self):
        ...
        self.step_count = 0
        self.means = [0.394, 0.409, 0.163, 0.066,
                      0.091, 0.097, 0.050, 0.057,
                      4.034, 20.600, 5.686, 4.021,
                      0.109, 0.163, 0.057, 0.03,
                      86.21, 42.25, 56.91, 163.15,
                      2.662, 12.500, 2.85, 2.31]
        self.baseline = False
    ...
    def start_cluster(self, workload, interference, hm_bad_nodes):
        ...
        if self.selected_workload in self.large:
            print("====> Sleeping for large object prepare stage..")
            time.sleep(60)
    ...
    def check_get_rows_and_get_obs(self, rows, job_id, url, fetch=False, fetch_count=0):
        ...
        try:
            ...
            fetch_unbalanced = len(rows) != 3 and self.selected_workload not in ["workload5.xml",
                                                                                 "workload6.xml"] and retries < 120
            fetch_balanced = self.selected_workload in ["workload5.xml",
                                                        "workload6.xml"] and len(rows) != 2 and retries < 120
    ...
    def get_cosbench(self):
        ...
        mean_idx = 4 * self.train_workloads.index(self.selected_workload) + 2 * self.idx_pair[0] + self.idx_pair[1]
