...
class ClusterEnvironment:
    ...
    def start_cluster(self, workload, interference, hm_bad_nodes):
        ...
        print('---->>>> RESET aff. and weight values to 1..')
        self.set_one()
        # self.change(np.ones(shape=(NUM_NODES,)), change_affinity=True)
        # self.change(np.ones(shape=(NUM_NODES,)), change_affinity=False)
    ...
    def change(self, new_values, change_affinity=1., baseline=False):
        ...
        for node_num, value in enumerate(new_values):
            ...
            command = 'ceph osd {} {} {}'.format(key, node_num, float(value))
            print(command)
    ...
    def set_one(self):
        new_values = np.ones(self.node_num)
        for node_num, value in enumerate(new_values):
            key = 'primary-affinity'
            command = 'ceph osd {} {} {}'.format(key, node_num, float(value))
            print(command + '\n')
            p = Popen(command, shell=True)
            p.communicate()
...
class DlrEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.baseline = False
        self.hm_bad_nodes = 2
        self.idx_pairs = [(0, 0), (0, 1), (1, 0), (1, 1)] # for alternate
        self.intf_idx = 0
    ...
    def reset(self):
        ...
        print('Sleeping for 180s..inside reset..after 1st stop cluster')
        time.sleep(180)
        ...
        if self.test:
            ...
            if self.test_count == 0:
                self.intf_idx = int(not self.intf_idx)

            selected_interf = self.env.interferences[self.intf_idx]
            self.workload = self.train_workloads[self.test_count]
            self.test_count += 1
            self.test_count %= len(self.train_workloads)
